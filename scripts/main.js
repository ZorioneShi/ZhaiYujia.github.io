;(function (window) {
  ...
  const modalBtnEle = document.getElementById('modalBtn')
  const letterBoxEle = document.getElementById('letter_box')
  ...
  function clickFun() {
    ...
    if (textIndex >= texts.length) {
      ...
      letterBoxEle.style.display = 'flex'
      letterBoxEle.style.opacity = '1'
      return
    }
  ...
  function event () {
    var isdisable = true;
    modalBtnEle.addEventListener('click', function (e) {
      if(isdisable){
        music1.play() // 播放音乐
        modalEle.setAttribute('type', 'hide')
        setTimeout(()=>{
          modalEle.style.display = 'none'
          contentEle.style.display = 'block'
          letterBoxEle.style.display = 'none'
        },3000)
        isdisable = false
      }
    }, false)
    contentEle.addEventListener('click', clickFun, false)
    contentEle.addEventListener('touchstart', clickFun, false)
  }
  function init () {
